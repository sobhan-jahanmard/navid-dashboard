    ...
    async jwt({ token, user, account, profile }) {
      ...
      if (user && account) {
        ...
        if (account.provider === 'discord' && profile && account.access_token) {
          console.log('Discord-specific profile:', JSON.stringify(profile, null, 2));
          ...
          try {
            const userRole = await determineUserRole(account.access_token, profile.id);
    ...
    async session({ session, token }) {
      ...
      if (token) {
        session.user = {
          ...
          name: token.username,
          image: token.discordAvatar ? `https://cdn.discordapp.com/avatars/${token.discordId}/${token.discordAvatar}.png` : null,
